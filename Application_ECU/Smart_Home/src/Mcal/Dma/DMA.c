...
DMA_StatusTypeDef DMA_Init(DMAcfg_t* psDMAcfg)
{
	...
	else
	{
		...
		u32 TempReg =__STREAM_REG(psDMAcfg -> BaseAddress , psDMAcfg->StreamNum)->DMA_SCR;
...
DMA_StatusTypeDef DMA_Start(DMAcfg_t* psDMAcfg,u32 pu32Src,u32 pu32Dest,u32 DataSize)
{
	DMA_StatusTypeDef Return_state = DMAOk;
	if(psDMAcfg == NULL)
	{
	...
	else
	{
		...
		__STREAM_REG(psDMAcfg->BaseAddress , psDMAcfg->StreamNum)->DMA_SNDTR = (u32)(DataSize &(0xffff));
